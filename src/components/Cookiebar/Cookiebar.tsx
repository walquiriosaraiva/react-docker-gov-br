...
const Cookiebar = () => {
  ...
  useEffect(() => {
    ...
    const getReport = (event: any) => {
      console.log(event.detail[0])
      element.setAttribute('hidden', '')
    }

    element.addEventListener('report', getReport)

    return () => {
      element.removeEventListener('report', getReport)
    }
  }, [])

  return <br-cookiebar ref={ref} lang={lang} content={JSON.stringify(content)}></br-cookiebar>
}
